...
reduction( int* data, unsigned int size) {
  ...
  sdata[threadIdx.x] = data[tid];
  sdata[threadIdx.x + blockDim.x] = data[tid + blockDim.x * gridDim.x];
  __syncthreads();

  unsigned int stride = 1;
  int csize = blockDim.x;

  while( stride < 2*blockDim.x) {
    if( threadIdx.x < csize) {
        unsigned int idx = 2 * stride * threadIdx.x;
...
main( int /*argc*/, char** /*argv*/ ) {
  ...
  int num_blocks = (size/2) / max_threads_per_block;
  if( 0 != (size/2) % max_threads_per_block) {
    num_blocks++;
  }
