...
////////////////////////////////////////////////////////////////////////////////////////////////////
void
initDeviceMemory( const std::vector<float>& data, float*& data_device, const unsigned int size) {

  // allocate device memory
  checkErrorsCuda( cudaMalloc((void **) &data_device, sizeof(float) * size));

  // copy device memory
  checkErrorsCuda( cudaMemcpy( data_device, &data[0], sizeof(float) * size, cudaMemcpyHostToDevice));
}
...
////////////////////////////////////////////////////////////////////////////////////////////////////
void
getResultDevice( const float* data_device, std::vector<float>& data, const unsigned int size) {

  checkErrorsCuda(cudaMemcpy( &data[0], data_device, sizeof(float) * size, cudaMemcpyDeviceToHost));
}
...
////////////////////////////////////////////////////////////////////////////////////////////////////
void
freeDeviceMemory( float*& data_device) {
...
////////////////////////////////////////////////////////////////////////////////////////////////////
__global__ void
reduction( float* data, unsigned int size, unsigned int stride) {
...
main( int /*argc*/, char** /*argv*/ ) {
  ...
  while( stride < size) {
    ...
    reduction<<< num_blocks , num_threads_per_block >>>( data_device, size, stride);
    stride *= 2;
  }
  checkLastCudaError( "Kernel launch failed.");

  getResultDevice( data_device, data, 1);
  int res = static_cast<int>(data[0]);
